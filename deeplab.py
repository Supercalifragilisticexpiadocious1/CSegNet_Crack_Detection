...
from model_utils import _SimpleSegmentationModel
...
class ECBAM(nn.Module):
    def __init__(self, channels, k_size=3):
        ...
        self.eca = ECAModule(channels, k_size)
        self.sa = SAM(channels, channels)
...
class DeepLabHeadV3Plus(nn.Module):
    ...
    def forward(self, feature):
        ...
        output_feature = F.interpolate(output_feature, size=low_level_feature.shape[2:], mode='bilinear',
                                       align_corners=False)
        concat_features = torch.cat([low_level_feature, output_feature], dim=1)
        if eca:
            return self.classifier(self.eca(concat_features))
        else:
